...
const ProfileCard = async (props) => {
  ...
  return (
    <div className='float-left h-full w-1/5 bg-white relative bottom-36 ml-16 rounded-xl shadow-2xl'>
      <Image src={profile.picture} width={150} height={200} alt="Profile Picture" className='rounded-full m-auto mt-8' />
      ...
      <Link href={profile.linkedIn} target='_blank' className='m-10 my-5 opacity-75 flex'><FaLinkedin className='text-2xl mr-1'/>LinkedIn</Link>
      <Link href={profile.gitHub} target='_blank' className='m-10 mt-5 opacity-75 flex'><FaGithub className='text-2xl mr-1'/>GitHub</Link>
      </div>
  )
}
