...
const GigCards = (props) => {
  ...
  return (
      <Link href={`/Gigs/${props.gigData._id}`} className='m-8 w-1/5 border rounded-2xl shadow-md hover:shadow-2xl cursor-pointer' onMouseEnter={() => setActive(true)} onMouseLeave={() => setActive(false)}>
     ...
     <button onClick={() => router.push(`/Gigs/${props.author.userId}`)} className='text-sm pt-2 px-1 font-semibold hover:underline'>{props.author.name}</button>
     </div>
